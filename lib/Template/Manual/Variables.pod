...
=head2 Automagic Promotion of Scalar to List
...
to list).  Class::DBI module is just one example of a paricularly 
...
than a list of 1 items) and leaves it well alone, returning the
...
item return is an object with a 'first' method, then that will be
called, as expected, in preference to the list virtual method.
