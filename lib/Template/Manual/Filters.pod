...
encoded (via the C<escape_html()> or C<encode_entities()> subroutines
respectively) to convert all extended characters into their
appropriate HTML entities (e.g. converting 'C<é>' to 'C<&eacute;>').  If
neither module is available on your system then an 'C<html_entity>' exception
