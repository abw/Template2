...
NOTE: this documentation may be a little flaky.
...
sequences that mark the start and end of a template directive.  The 
TAG_STYLE may be specified to set both according to a particular 
style.  Styles include 'metatext', 'html', 'mason', 'php', 'asp', and
so on.
...
Forces directive types to be in UPPER CASE.  Allows lower case reserved 
words to be used as variable names.  Disabled by default.
