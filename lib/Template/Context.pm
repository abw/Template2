...
=head1 SYNOPSIS
    ...
    # fetch (load and create) a filter sub-routine
    ...
    $exception = $context->catch($exception, \$output);

    # save/restore the stash to effect variable localisation
    $stash = $context->localise(\%vars);
    $stash = $context->delocalise();

    # AUTOLOAD method for retrieving internal references, flags, etc.
    $stash     = $context->stash();
    $tflag     = $context->trim();
...
context in which templates are processed.

=head1 METHODS

=head2 new(\%params) 

Constructor method inherited from Template::Base.  Calls the private
_init() method to perform object initialisation.
...
A reference to a Template::Stash object, or sub-class, which will
manage template variables for the context.  If unspecified, a default
Template::Stash is created using the VARIABLES or PRE_DEFINE configuration
items (if defined) to initialise the stash variables.

    my $context = Template::Context->new({
	STASH => MyTemplate::Stash({ foo => 'bar' })
    });

=item VARIABLES, PRE_DEFINE

The VARIABLES option (or PRE_DEFINE - they're equivalent) can be used to 
specify a hash array of template variables that should be used to 
pre-initialise the Template::Stash when it is created.  These items are
ignored if STASH is already defined by the above.
...
A reference to a list of Template::Provider objects, or sub-classes thereof,
which are responsible for loading and compiling templates. 
...
If undefined, a default Template::Provider is created using the
current configuration parameters.  For example, the Template::Provider
INCLUDE_PATH option can be specified in the Template::Context
configuration and will be correctly passed to the constructor.
...
A reference to a list of provider objects (i.e. they implement
fetch()) which are responsible for loading and instantiating template
plugin objects.  

    my $context = Template::Context->new({
	PLUGINS => [
    	    MyTemplate::PluginLoader->new({ ... }),
    	    Template::Plugins->new({ ... }),
	],
    });

By default, a single Template::Plugins object is created using the 
current configuration hash.  See L<Template::Plugins> for details of
relevant configuration items.

    my $context = Template::Context->new({
	PLUGIN_BASE => 'MyTemplate::Plugins',
	LOAD_PERL   => 1,
    });
...
A reference to a list of provider objects (i.e. they implement
fetch()) which are responsible for loading and creating filter sub-routines.
...
By default, a single Template::Filters object is created for the FILTERS
list.

=item EVAL_PERL

Flag to indicate if PERL blocks should be evaluated by this context.
Disable by default (Perl code not evaluated).

=item TRIM

Flag to indicate if template output should have leading or trailing
whitespace removed.  Disabled by default (no trimming).
...
to load plugin modules, according the various configuration items such 
as PLUGIN_BASE, LOAD_PERL, etc., and then instantiate an object via new().
A reference to a list of constructor arguments may be passed as the second
parameter.  These are forwarded to the constructor.  Returns a reference 
to a plugin (which is generally an object, but doesn't have to be) or 
undef on failure.  Errors can be retrieved via error().
...
Instantiates a filter sub-routine by querying the FILTERS providers.
...
use.  The filter is only cached if an alias is provided, or if the
filter was created without additional arguments.  The filter is cached
under its own $name if $alias is undefined.  Subsequent calls to
filter($name) will return the cached entry, if defined.  Specifying
arguments bypasses the caching mechanism and always creates a new
filter.
...
buffer.  This is then stored in the exception object when creating,
...
=head2 $context->catch($exception, \$output)

Catches an exception thrown, either as a reference to a Template::Exception
object or some other value.  In the latter case, the error string is 
promoted to a Template::Exception object of 'undef' type.  This method also
...
reference).  By this process, the full output buffer can be reconstructed
regardless of where an exception is thrown from.
