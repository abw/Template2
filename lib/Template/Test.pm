...
$DEBUG   = 0;
@ISA     = qw( Exporter );
@EXPORT  = qw( ntests ok flush test_expect callsign );
$| = 1;
...
sub-routines which can be used to automate test scripts for the
Template Toolkit.  See the numerous tests in the 't' sub-directory of
the distribution for examples of use.

The test_expect() sub-routine splits an input document into a number
...
'callsign' sub-routine which returns a hash mapping a..z to their phonetic 
